require '/home/dell/ruby_Kylas/jigar/shop/shopcart_class'
...
describe Shop do
  ...
  describe "#add_product_to_stock:" do
    it "it should add the new product in the stock" do
      ...
      obj.add_product_to_stock
      expect(obj.stock).to eq({"Oneplus buds" => {:description => "12.4 mm driver size", :price => 1500.0, :quantity => 20}})
    end
  end
  ...
  describe "#list_all_products:" do
    context "when stock is empty" do
      it "then it should give an error message" do
        expect{obj.list_all_products}.to output("\nStock is empty\n").to_stdout
      end
    end
    ...
    context "when there's enough quantity of product in the stock" do
      ...
      context "and the product has already added in the cart" do
        before do
          ...
          obj.cart = {
            "SONY TV" => {
              :description => obj.stock["SONY TV"][:description],
              :price => obj.stock["SONY TV"][:price], 
              :quantity => 10
            }
          }
        end
  ...
  describe "#generate_bill:" do
    ...
    context "when cart is empty" do
      it "then it should give an error message" do
        expect {obj.generate_bill}.to output("\nCart is empty\n").to_stdout
      end
    end
